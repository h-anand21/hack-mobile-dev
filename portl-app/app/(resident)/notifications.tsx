...
import { ArrowLeft, Bell, Users, Megaphone, CreditCard, ChevronRight } from 'lucide-react-native';
...
export default function NotificationsScreen() {
  ...
  const getIcon = (iconType: string, color: string) => {
    switch (iconType) {
      ...
      case 'megaphone': return <Megaphone size={20} color={color} />;
      case 'creditcard': return <CreditCard size={20} color={color} />;
    ...
    <SafeAreaView className="flex-1 bg-[#F8F9FB]">
      ...
      <View className="flex-row justify-between items-center px-5 pt-3 pb-4 bg-white border-b border-gray-100">
        ...
        <View className="items-center">
          <Text className="text-gray-900 font-extrabold text-lg">Notifications</Text>
          <Text className="text-gray-400 text-xs font-semibold">Stay alert with real-time updates</Text>
        </View>
      ...
      >
        ...
        <View className="bg-white p-1.5 rounded-2xl flex-row justify-between mb-5 shadow-sm border border-gray-100">
          <TouchableOpacity
            ...
            className={`flex-1 py-2.5 rounded-xl items-center ${activeFilter === 'all' ? 'bg-gray-900' : ''}`}
          >
          ...
          <TouchableOpacity
            ...
            className={`flex-1 py-2.5 rounded-xl items-center ${activeFilter === 'approvals' ? 'bg-gray-900' : ''}`}
          >
          ...
          <TouchableOpacity
            ...
            className={`flex-1 py-2.5 rounded-xl items-center ${activeFilter === 'society' ? 'bg-gray-900' : ''}`}
          >
        ...
        {filtered.map((item, idx) => (
          <Animated.View 
            ...
            className={`p-4 rounded-3xl mb-3 border flex-row items-start justify-between ${
            ...
            <View className="flex-row items-start flex-1 pr-2">
              ...
              <View className="flex-1">
                <View className="flex-row items-center justify-between">
                  <Text className="text-gray-900 font-extrabold text-sm flex-1 pr-2">{item.title}</Text>
                  {!item.read && (
